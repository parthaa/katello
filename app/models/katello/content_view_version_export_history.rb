module Katello
  class ContentViewVersionExportHistory < Katello::Model
    ...
    def self.pick_recent_history(content_view, destination_server)
      recent_history_id = where(content_view_version: content_view.versions, destination_server: destination_server).maximum(:id)
      find(recent_history_id)
    end
  end
end
