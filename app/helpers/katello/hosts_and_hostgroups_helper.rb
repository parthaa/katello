module Katello
  module HostsAndHostgroupsHelper
    ...
    def kickstart_repository_id(host, options = {})
      ...
      return selected_host_group.kickstart_repository_id if selected_host_group.try(:kickstart_repository_id).present?
      ...
      if selected_host_group.try(:medium_id).blank? && host.try(:medium_id).blank?
        kickstart_repository_options(host, options).first.try(:id)
      end
    end
    ...
    def kickstart_repository_options(param_host, options = {})
      ...
      if param_host.present?
        ...
        new_host = ::Host.new
        new_host.operatingsystem = host.operatingsystem
        new_host.architecture = host.architecture
