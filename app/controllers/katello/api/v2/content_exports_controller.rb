module Katello
  class Api::V2::ContentExportsController < Api::V2::ApiController
    ...
    api :GET, "/content_exports", N_("Show the export history for a content view version")
    ...
    def index
      ...
      history = history.where(:id => params[:export_history_id]) unless params[:export_history_id].blank?
    ...
    def find_exportable_content_view_version
      @version = ContentViewVersion.exportable.find(params[:id])
      throw_resource_not_found(name: 'content_view_version', id: params[:id]) if @version.blank?
    end
  end
end
