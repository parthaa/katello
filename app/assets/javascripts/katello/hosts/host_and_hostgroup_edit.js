...
KT.hosts.toggle_installation_medium = function() {
    ...
    if (content_view_id && lifecycle_environment_id && content_source_id && architecture_id && operatingsystem_id) {
        $("select[data-kickstart-repository-id] > option").remove();
        $.ajax({
            type:'get',
            ...
            success: function(result){
                if (result.length > 0) {
                    KT.hosts.update_media_type_selection(false);
                    _.each(result, function(val) {
                        var newOption = $('<option>');
                        newOption.attr('value',val.id).text(val.name);
                         $('select[data-kickstart-repository-id]').append(newOption);
                    });
                    $('select[data-kickstart-repository-id]').find('option:eq(0)').prop('selected', true);
                } else {
                    KT.hosts.update_media_type_selection(true);
                };
            }
        })
    } else {
...
KT.hosts.on_install_media_dropdown_change = function() {
    ...
    $('select[data-kickstart-repository-id]').val("");
    activate_select2("#media_select");
};
...
KT.hosts.set_synced_content_bindings = function() {
    $('select[data-kickstart-repository-id]').change(KT.hosts.on_synced_content_dropdown_change);
};
