    ...
    module ContentViewVersion
      class MetadataMap
        ...
        def initialize(metadata:)
          @toc = metadata[:toc]
          @products = parse_products(metadata[:products])
          @repositories = parse_repositories(metadata[:repositories])
          @content_view = parse_content_view(metadata[:content_view])
          @content_view_version = parse_content_view_version(metadata[:content_view_version])
        ...
        def parse_products(products)
          products.values.map do |p|
            MetadataProduct.new(
              ...
              #gpg_key: p[:gpg_key] ? MetadataGpgKey.new(name: p[:gpg_key][:name]) : nil, # Map this from @gpg_keys
              redhat: ::Foreman::Cast.to_bool(p[:redhat]) # cast to bool...
              # TODO more fields?
            )
          end
        end
        ...
        def parse_repositories(repositories)
          repositories.values.map do |repo|
            ...
            raise _("Invalid repository in the metadata #{repo} error=#{e.message}")
          end
        end
