...
module ::Actions::Katello::ContentViewVersion
  ...
  class ExportTest < TestBase
    ...
    it 'plans' do
      ...
      assert_action_planned_with(action, ::Actions::Pulp3::ContentViewVersion::Export,
                                         content_view_version_id: content_view_version.id,
                                         smart_proxy_id: master.id,
                                         export_base_dir: export_dir)

      assert_action_planned_with(action, ::Actions::Pulp3::ContentViewVersion::DestroyExporter) do |args|
        args = args.first
        assert_equal master.id, args[:smart_proxy_id]
        assert args[:exporter_data].present?
      end
    end
  end
end
