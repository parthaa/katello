...
class HostsAndHostGroupsHelperKickstartRepositoryIDTest < HostsAndHostGroupsHelperTestBase
  ...
  test "must handle selected hosts or host groups" do
    ...
    assert_equal id, kickstart_repository_id(nil, :selected_host_group => @hostgroup)
    # even in the case of a non null host if the host group got selected you want it use
    # the KS id specfied by new host group
    ...
    assert_nil kickstart_repository_id(nil, :selected_host_group => @hostgroup)
  end
end
